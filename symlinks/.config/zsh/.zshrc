# source /run/current-system/sw/etc/profile.d/nix.sh # https://wiki.nixos.org/wiki/Nix_Installation_Guide
...
export EDITOR=kak RIDE_JS=~/.config/Ride-4.5/vim.js FLAKE=$(readlink -f ~/.dotfiles)
alias n=nix \
      g=git \
      x=exec \
      t=tmux \
      ...
      gs=' git -C ~/.dotfiles submodule foreach git s -bs' \
      dy=' CONFIGFILE=~/.config/dyalog/u.dcfg rlwrap -a mapl -b -s'
